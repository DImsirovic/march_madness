...
test_mean <- function(null_dist, mu) {
    values <- mu + rnorm(3500 * 0.8, mean = 37, sd = 10)
    not_seeded <- data.frame(values) %>% mutate(seeded = FALSE)
    
    values <- rnorm(3500 * 0.2, mean = 37, sd = 10)
    seeded <- data.frame(values) %>% mutate(seeded = TRUE)
    
    sim_adjEM <- rbind(seeded, not_seeded)
    obs_diff <- mean_diff(sim_adjEM$values, sim_adjEM$seeded)
...
diffs <- seq(from = 0, to = 3, length.out = 20)
powers <- rep(0, 20)
